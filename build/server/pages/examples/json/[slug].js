...
;// CONCATENATED MODULE: ./api/json-api.ts


const SEP = "@";
const JSON_PATH = "/chunjun-examples/json";
const jsonDirectory = (0,external_path_.join)(process.cwd(), JSON_PATH);
const getAllJsonPaths = (root = JSON_PATH, allFiles = [])=>{
    const currentDir = (0,external_path_.join)(process.cwd(), root);
    const files = external_fs_default().readdirSync(currentDir);
    for (const file of files){
        if (file.includes(".")) {
            allFiles.push(root === JSON_PATH ? file : `${root.split("/").slice(3).join("/")}/${file}`);
        } else {
...
const getJsonByName = (name)=>{
    const realSlug = name.split(SEP).join("/").replace(/\.json$/, "");
    const fullPath = (0,external_path_.join)(jsonDirectory, `${realSlug}.json`);
...
;// CONCATENATED MODULE: ./pages/examples/json/[slug].tsx
...
/* harmony default export */ const _slug_ = (JsonExamples);
...
var __webpack_exports__ = __webpack_require__.X(0, [707,533,706,546,629], () => (__webpack_exec__(4020)));
