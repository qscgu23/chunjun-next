...
const Post = ({ content, tree }: { content: string; tree: FileTree[] }) => {
  ...
  return (
    <DocumentLike tree={tree} target="/documents">
      {router.isFallback ? <Skeleton visible className="md:col-span-4" /> : <PostBody content={content} />}
    </DocumentLike>
  );
};
  ...
  const generate = (path: string, obj: FileTree[]): FileTree[] => {
    if (!path.includes("/")) {
      ...
      if (!f) {
        ...
        return generate(current, obj.find((o) => o.label === t)?.children as FileTree[]);
      } else {
  ...
  for (const m of map) {
    ...
    for (const link of allPaths) {
      if (link.split("/")[0] === root.label) {
        generate(link.split("/").slice(1).join("/"), root.children as FileTree[]);
      }
    }
    tree.push(root);
  }
